import React, { useState, useEffect } from "react";
import {
  View,
  Text,
  StatusBar,
  ScrollView,
  TouchableOpacity,
  SafeAreaView,
  Image,
  StyleSheet,
  Linking,
  Share,
  Clipboard,  // Import Clipboard
} from "react-native";
import COLOURS from "../../Constants/Colors";
import Entypo from "react-native-vector-icons/Entypo";
import FontAwesome from "react-native-vector-icons/FontAwesome";
import styles from "./styles";
import axios from "axios";
import { getDatabase, ref, remove,update } from "firebase/database";
import { db } from "../../../firebase";
const RecipeDetailScreen = ({ route, navigation, isDarkTheme }) => {
  const [recipe, setRecipe] = useState({});
  const { randomMeal,searchMeal } = route.params;
  const backgroundColor = isDarkTheme ? "black" : "white";
  const textStyle = isDarkTheme ? styles.darkText : styles.lightText;

  const openYouTube = () => {
    const url = recipe.strYoutube;
    Linking.canOpenURL(url)
      .then((supported) => {
        if (supported) {
          Linking.openURL(url);
        } else {
          console.log("Don't know how to open this URL: " + url);
        }
      })
      .catch((err) => console.error("An error occurred", err));
  };

  const shareRecipeVideo = () => {
    const url = recipe.strYoutube;
    const message = `Check out this amazing recipe video: ${url}`;

    Share.share({
      message: message,
      url: url,
      title: "Amazing Recipe Video",
    })
      .then((result) => {
        console.log(result);
      })
      .catch((error) => {
        console.error("Error sharing", error);
      });
  };

  const copyRecipeDetails = () => {
    const details = `
      Recipe: ${recipe.strMeal}
      Category: ${recipe.strCategory}
      Cuisine: ${recipe.strArea}
      Ingredients:
      ${Object.keys(recipe)
        .filter(key => key.startsWith("strIngredient") && recipe[key])
        .map(key => `- ${recipe[key]}: ${recipe[`strMeasure${key.slice("strIngredient".length)}`]}`)
        .join('\n')}
      
      Instructions:
      ${recipe.strInstructions}
    `;

    Clipboard.setString(details);
    alert('Recipe details copied to clipboard!');
  };

  useEffect(() => {
   

    if (randomMeal) {
      fetchRandomRecipe();
    } else {
      // fetchRecipe();
      const { product} = route.params;
      setRecipe(product)
    }
  }, []);

  // const fetchRecipe = async () => {
  //   try {
  //     const { productID } = route.params;
  //     const response = await axios.get(
  //       `https://www.themealdb.com/api/json/v1/1/lookup.php?i=${productID}`
  //     );
  //     setRecipe(response.data.meals[0]);
  //   } catch (error) {
  //     console.error(error);
  //   }
  // };
  const deleteRecipe = async () => {
    try {
      const { product} = route.params;
      const key=product.key; 
      if (key) {
        const dbRef = ref(db, `/${key}`);
        await remove(dbRef);
        alert('Recipe deleted successfully');
        navigation.goBack();
      } else {
        alert('Unable to delete recipe. Missing recipe key.');
      }
    } catch (error) {
      console.error("Error deleting recipe:", error);
    }
  };

  const fetchRandomRecipe = async () => {
    try {
      const response = await axios.get(
        `https://www.themealdb.com/api/json/v1/1/random.php`
      );
      setRecipe(response.data.meals[0]);
    } catch (error) {
      console.error(error);
    }
  };
  const toggleFavourite = async () => {
    try {
      const { product} = route.params;
      const key=product.key;  
      if (key) {
        const dbRef = ref(db, `/${key}`);
        const updatedFavouriteStatus = !recipe.isFavourite;
        await update(dbRef, { isFavourite: updatedFavouriteStatus });
        alert(`Recipe ${updatedFavouriteStatus ? 'added to' : 'removed from'} favourites`);
        setRecipe(prevState => ({ ...prevState, isFavourite: updatedFavouriteStatus }));
      } else {
        alert('Unable to update favorite status. Missing recipe key.');
      }
    } catch (error) {
      console.error("Error updating favorite status:", error);
    }
  };
  return (
    <SafeAreaView style={styles.safeArea} backgroundColor={backgroundColor}>
      <StatusBar
        backgroundColor={COLOURS.backgroundLight}
        barStyle="dark-content"
      />
      <ScrollView style={styles.container}>
        <View style={styles.imageContainer}>
          <View
            style={styles.headerContainer}
            backgroundColor={backgroundColor}
          >
            <TouchableOpacity onPress={() => navigation.goBack("Home")}>
              <Entypo name="chevron-left" style={styles.backButton} />
            </TouchableOpacity>
          </View>
        </View>

        <Image source={{ uri: recipe.strMealThumb }} style={styles.image} />
        <View style={styles.detailContainer}>
          <Text style={[styles.title, textStyle]}>{recipe.strMeal}</Text>

          <View style={styles.infoContainer}>
            <Text style={[styles.infoTitle, textStyle]}>Category:</Text>
            <Text style={[styles.infoContent, textStyle]}>
              {recipe.strCategory}
            </Text>
          </View>

          <View style={styles.infoContainer}>
            <Text style={[styles.infoTitle, textStyle]}>Cuisine:</Text>
            <Text style={[styles.infoContent, textStyle]}>
              {recipe.strArea}
            </Text>
          </View>

          <Text style={[styles.subTitle, textStyle]}>Ingredients</Text>
          {Object.keys(recipe).map((key) => {
            if (key.startsWith("strIngredient") && recipe[key]) {
              const measureKey = `strMeasure${key.slice(
                "strIngredient".length
              )}`;
              return (
                <Text key={key} style={[styles.ingredient, textStyle]}>
                  - {recipe[key]}: {recipe[measureKey]}
                </Text>
              );
            }
          })}

          <Text style={[styles.subTitle, textStyle]}>Instructions</Text>
          <Text style={[styles.instructions, textStyle]}>
            {recipe.strInstructions}
          </Text>

          <View style={styles.buttonContainer}>
            <TouchableOpacity
              onPress={openYouTube}
              style={[styles.button, styles.watchButton]}
            >
              <FontAwesome
                name="youtube-play"
                size={18}
                color="#ffffff"
                style={styles.icon}
              />
              <Text style={styles.buttonText}>Watch Video</Text>
            </TouchableOpacity>
            <TouchableOpacity
              onPress={shareRecipeVideo}
              style={[styles.button, styles.shareButton]}
            >
              <FontAwesome
                name="share"
                size={18}
                color="#ffffff"
                style={styles.icon}
              />
              <Text style={styles.buttonText}>Share</Text>
            </TouchableOpacity>
            <TouchableOpacity
              onPress={copyRecipeDetails}
              style={[styles.button, styles.copyButton]}
            >
              <FontAwesome
                name="clipboard"
                size={18}
                color="#ffffff"
                style={styles.icon}
              />
              <Text style={styles.buttonText}>Copy Details</Text>
            </TouchableOpacity>
            </View>
          {(randomMeal==null) && (searchMeal==null) &&
            (<View style={styles.buttonContainer}><TouchableOpacity
              onPress={deleteRecipe}
              style={[styles.button, styles.deleteButton]}
            >
              <FontAwesome
                name="trash"
                size={18}
                color="#ffffff"
                style={styles.icon}
              />
              <Text style={styles.buttonText}>Delete Recipe</Text>
            </TouchableOpacity>
            <TouchableOpacity
              onPress={toggleFavourite}
              style={ styles.favouriteButton}
            >
              <FontAwesome
                name={recipe.isFavourite ? "star" : "star-o"}
                size={18}
                color="#ffffff"
                style={styles.icon}
              />
              <Text style={styles.buttonText}>
                {recipe.isFavourite ? "Remove it" : "Add to Favourites"}
              </Text>
            </TouchableOpacity></View>)
}
          
        </View>
      </ScrollView>
    </SafeAreaView>
  );
};

export default RecipeDetailScreen;
